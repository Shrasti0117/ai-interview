...
function SubjectCard({ subject, onSelect, view }) {
  ...
  const renderIcon = (icon) => {
    ...
    if (icon.startsWith("fa")) {
      ...
      const faIcon = Icons[iconName] || Icons.faBook;
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ display: "flex", gap: 14, minWidth: 0, flex: 1 }}>
          <div style={{ width: 50, height: 50, background: subject.color, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0, color: subject.iconColor }}>
            {renderIcon(subject.icon)}
          </div>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 800, color: "#1a1a2e", lineHeight: 1.3, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{subject.name}</div>
...
export default function SubjectsPage() {
  ...
  const fetchDbSubjects = async () => {
    try {
      ...
      const mapped = res.data.data.map(s => ({
        id: s._id,
        name: s.title,
        short: s.title.split(' ').map(w => w[0]).join('').toUpperCase(),
        icon: s.icon || "📚",
        color: s.color || "#e8f0fe",
        iconColor: s.iconColor || "#1a73e8",
        borderColor: s.iconColor || "#1a73e8",
        progress: 0,
        totalTopics: 5,
        doneTopics: 0,
        difficulty: "Medium",
  ...
  const allFilteredSubjects = [...allSubjects, ...dbSubjects];
  ...
  // Stats
  const started = allFilteredSubjects.filter(s => s.progress > 0).length;
  const completed = allFilteredSubjects.filter(s => s.progress === 100).length;
  const overallProgress = Math.round(allFilteredSubjects.reduce((acc, s) => acc + s.progress, 0) / (allFilteredSubjects.length || 1));
    ...
    <div style={{ background: "#f0f4ff", minHeight: "100vh", fontFamily: "'Sora', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: "36px 20px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 32 }}>
          {[
            { label: "Total Subjects", val: allFilteredSubjects.length, color: "#1a73e8", bg: "#e8f0fe", icon: "📖" },
        ...
        {/* Dynamic Admin Subjects Section */}
        {dbSubjects.length > 0 && (
          <div style={{ marginBottom: 40 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              <div style={{ width: 5, height: 28, background: "#1e8e3e", borderRadius: 99 }}></div>
              <h2 style={{ fontSize: 20, fontWeight: 800, margin: 0, color: "#1a1a2e" }}>Admin Added Subjects</h2>
              <span style={{ background: "#e6f4ea", color: "#1e8e3e", fontSize: 12, fontWeight: 800, padding: "2px 10px", borderRadius: 99 }}>{dbSubjects.length}</span>
            </div>
            <div style={{ 
              display: view === "grid" ? "grid" : "flex", 
              flexDirection: view === "grid" ? "initial" : "column",
              ...
              {applyFilters(dbSubjects).map(s => <SubjectCard key={s.id} subject={s} onSelect={setSelected} view={view} />)}
            </div>
          </div>
        )}
