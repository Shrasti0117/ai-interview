...
function SubjectModal({ subject, onClose, onContinue }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1000,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"32px 20px",overflowY:"auto"}} onClick={handleClose}>
      <div className="slide-up" style={{background:"#fff",borderRadius:24,padding:"28px 24px",maxWidth:520,width:"100%",position:"relative",boxShadow:"0 24px 64px rgba(0,0,0,.18)",maxHeight:"calc(100vh - 64px)",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <button onClick={(e) => { e.stopPropagation(); handleClose(); }} style={{position:"absolute",top:16,right:16,width:32,height:32,borderRadius:"50%",border:"1px solid #e5e7eb",background:"#f9fafb",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",color:"#555"}}>✕</button>
...
function SubjectsPageContent({ onSelectSubject }) {
  ...
  const focusSubject = new URLSearchParams(location.search).get('subject');

  useEffect(() => {
    if (!focusSubject) return;
    const value = decodeURIComponent(focusSubject);
    const foundById = all.find(s => s.id === value);
    const foundByName = all.find(s => s.name === value || s.name.toLowerCase() === value.toLowerCase());
