...
export default function Dashboard() {
  ...
  const fetchAdminTasks = async () => {
    try {
      const res = await axios.get("http://localhost:5001/api/admin/planner", {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      });
      const adminTasks = res.data.data.map(t => ({
        id: t._id,
        text: t.task,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {tasks.map(task => (
                  <div key={task.id} style={{ 
                    ...
                    padding: "12px", 
                    borderRadius: 12, 
                    background: task.isAdmin ? "#fffbeb" : (task.completed ? "#f8f9fa" : "#f0f4ff"), 
                    border: task.isAdmin ? "1px solid #fbbf24" : "1px solid #e8eaed" 
                  }}>
                    {task.isAdmin && <span style={{fontSize: 10, background: '#fbbf24', padding: '2px 6px', borderRadius: 4, fontWeight: 800}}>ADMIN</span>}
                    <input 
                      type="checkbox" 
                    ...
                    {!task.isAdmin && <button onClick={() => deleteTask(task.id)} style={{ background: "transparent", border: "none", color: "#9aa0a6", cursor: "pointer", fontSize: 16 }}>×</button>}
                  </div>
                ))}
              </div>
            </div>
